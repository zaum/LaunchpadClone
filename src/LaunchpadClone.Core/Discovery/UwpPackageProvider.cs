...
public sealed class UwpPackageProvider : IUwpPackageProvider
{
    ...
    public async Task<List<AppItem>> GetInstalledAppsAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // NOTE: PackageManager does not implement IDisposable in the
            // CsWinRT projection — do not wrap it in a using statement.
            var manager = new PackageManager();
            packages = manager.FindPackagesForUser(string.Empty)
                .Where(p => !p.IsFramework && !p.IsResourcePackage
                            && p.SignatureKind != Windows.ApplicationModel.PackageSignatureKind.System)
                .ToList();
        }
        catch (Exception)
        {
    ...
    private async Task TrySaveLogoAsync(Windows.ApplicationModel.Core.AppListEntry entry, string id, CancellationToken ct)
    {
        ...
        try
        {
            var logoRef = entry.DisplayInfo.GetLogo(new Windows.Foundation.Size(256, 256));
            // The WinRT stream implements only IDisposable (IClosable) in the
            // projection — a plain using, not await using.
            using var randomAccessStream = await logoRef.OpenReadAsync();
            await using var fileStream = File.Create(cachePath);
            await randomAccessStream.AsStreamForRead().CopyToAsync(fileStream, ct);
        }
        catch (Exception)
        {
